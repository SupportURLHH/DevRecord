...
- (void)setInitialSettings{
    ...
    _textColor     = [UIColor colorWithRGB:0x333333];
...
- (void)createScrollLabelNeedStandbyLabel:(BOOL)isNeed{
    _currentScrollLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width-50, self.frame.size.height)];
    ...
    _currentScrollLabel.numberOfLines = 0;
    [self addSubview:_currentScrollLabel];
    
    if (isNeed) {
        _standbyScrollLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, -100, self.frame.size.width-50, self.frame.size.height)];
        ...
        _standbyScrollLabel.numberOfLines = 0;
